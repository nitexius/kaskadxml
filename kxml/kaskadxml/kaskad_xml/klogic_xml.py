...
def get_group_tags(tag_groups: Element) -> List[str]:
    ...
    if tag_groups.attrib['Name'] == 'Alarms' and len(tag_groups) > c.central_alarm_len:
        for alarm_number, in_out_group in enumerate(tag_groups):
            try:
...
class KlogicXML:
    ...
    def shift(self) -> ShiftAttrs:
        ...
        for group in self.module[1:]:
            if all([
                ...
                len_group = len(group)
                shift_attr.all_lens.add(len_group)
    ...
    def get_current_inout(self, fb_input: int, noffl_number: int):
        """Поиск группы контроллера в Klogic XML"""
        kl_find = self.klogic_tree_find()
        groups = kl_find.Groups
        all_fb_input_number = get_all_input_number(fb_input, noffl_number)

        if fb_input + 1 == self.len_fb:  # последний вход ФБ
            return 'continue'
        if all_fb_input_number < self.len_groups:  # проверка, чтобы текущий номер входа ФБ не превышал кол-во контроллеров
    ...
    def get_noffl_all_tags_info(self, fb: Element, noffl_number: int, good_tags: Iterable):
        ...
        for fb_input, fb_group in enumerate(fb, i.service_inputs):  # 3 входа ФБ не используются на этом этапе

            if fb_input == i.last_noffl_input:
                break

            inout = self.get_current_inout(fb_input, noffl_number)
            if inout == 'continue':
                continue
            if inout == 'break':
                break
            ...
            for noffl_number in range(0, 15):
                if fblock.text == f'noffl {noffl_number + 1}':
                    ...
                    for noffl_input in fb[i.first_fb_input:]:
                        if self.noffl_contr + i.first_contr < self.len_groups:  # len(groups) - общее количество групп в Klogic XML, включая служебные(3 шт.)
